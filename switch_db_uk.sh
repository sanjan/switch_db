...
mtdb=\$(grep "fr1a2pasedbvcs:5000/a2p_msgdb_fr1" $mtconf | wc -l)
if [[ \$mtdb -eq 1 ]]
then
...
local options=("Set MT/NOTIF ==> to UK4 DB"
               "Set MO ==> to UK4 DB"
               "Set MT/NOTIF ==> to FR1 DB"
                           "Set MO ==> to FR1 DB"
...
case $REPLY in
        ...
        2)      f_switch "MO" "UK" "${morouters_uk_main[@]}"
                f_anykey
                break ;;

        3)      f_switch "MT" "FR" "${mtrouters_uk_main[@]}"
        ...
        5)      f_switch_selected_mt "UK"
                f_anykey
                break ;;

        6)      f_switch_selected_mt "FR"
                f_anykey
                break ;;
